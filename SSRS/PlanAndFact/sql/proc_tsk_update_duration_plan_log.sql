...
/****** Object:  StoredProcedure [dbo].[TSK_UpdateDurationPlanLog]    Script Date: 06.02.2025 16:07:36 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[TSK_UpdateDurationPlanLog]
...
BEGIN
	...
	DELETE FROM [TSK_DURATION_PLAN_LOG] WHERE [TASK_ID] = @TASK_ID and ([DATE] < @START_PLAN  OR [DATE] > @END_PLAN)

	IF (@IS_NEED_ADD_FULL_TIME = 0)
	BEGIN
		SET @FIRST_DAY_CUR_MONTH = DATEADD(DAY,1,EOMONTH(GETDATE(),-1))
		SELECT @DURATION_BEFORE_FIRST_DAY_CUR_MONTH = SUM(DURATION) FROM [TSK_DURATION_PLAN_LOG]
	...
	BEGIN
		...
		WHILE (@DATE_CURSOR <= @END_PLAN)
			...
			BEGIN
					UPDATE [TSK_DURATION_PLAN_LOG] SET [DURATION] = @AVERAGE_DURATION WHERE TASK_ID = @TASK_ID AND [DATE] = @DATE_CURSOR
		
					IF (@@ROWCOUNT = 0)
					BEGIN
						INSERT INTO [TSK_DURATION_PLAN_LOG] ([TASK_ID] ,[DURATION] ,[DATE])
						VALUES (@TASK_ID, @AVERAGE_DURATION, @DATE_CURSOR)
					END
			END
			ELSE
			BEGIN
				DELETE [TSK_DURATION_PLAN_LOG] WHERE TASK_ID = @TASK_ID AND [DATE] = @DATE_CURSOR
			END
